...
namespace nl {
	...
	node::node(nodedata const * d, filedata const * f): 
		m_data(d), m_file(f) {
	}
	...
	node node::get(std::string const & o) const {
		return (*this)[o];
	}
	/*
	...
	*/
	...
	vector2i node::get_vector(vector2i def) const {
		if (m_data && m_data->type == nodetype::vector)
			return to_vector();
		return def;
	}
	/*
	...
	*/
	int32_t node::x() const {
		return m_data && m_data->type == nodetype::vector ? m_data->vector[0] : 0;
	}
	int32_t node::y() const {
		return m_data && m_data->type == nodetype::vector ? m_data->vector[1] : 0;
	}
	...
	nodetype node::data_type() const {
		return m_data ? m_data->type : nodetype::none;
	}
