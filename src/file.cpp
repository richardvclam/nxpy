...
#endif
		m_data->header = reinterpret_cast<fileheader const *>(m_data->base);
		if (m_data->header->magic != 0x34474B50)
			throw std::runtime_error(name + " is not a PKG4 NX file");
		m_data->node_table = reinterpret_cast<nodedata const *>(reinterpret_cast<char const *>(m_data->base) + m_data->header->node_offset);
